...
class NodeManager:
    ...
    #      create_chunk_node_and_link_to_source, set_chunk_content_embedding - remain the same) ...
    async def create_or_merge_source_node(
        self,
        identifier: str,
        content: Optional[str],
        dynamic_metadata: Optional[Dict[str, Any]],
        created_at: datetime
    ) -> Optional[str]:
        source_uuid = str(uuid.uuid5(uuid.NAMESPACE_DNS, identifier))
        params = {
            "source_identifier_param": identifier,
        ...
        try:
            ...
            if results and results[0]["source_uuid"]:
                return results[0]["source_uuid"]
            logger.warning(f"NodeManager: MERGE_SOURCE_NODE for '{identifier}' did not return a UUID.")
            return None
    ...
    async def set_source_content_embedding(self, source_uuid: str, embedding_vector: List[float]) -> bool:
        try:
            ...
            results, _, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.SET_SOURCE_CONTENT_EMBEDDING, params, database_=self.database
            )
    ...
    async def create_chunk_node_and_link_to_source(
        self,
        chunk_uuid: str,
        chunk_content: str,
        ...
        created_at: datetime,
        dynamic_chunk_properties: Dict[str, Any], 
        chunk_number_for_rel: Optional[int]
    ) -> Optional[str]:
        params = {
            "source_node_uuid_param": source_node_uuid,
            "chunk_uuid_param": chunk_uuid,
            "chunk_content_param": chunk_content,
            "source_identifier_param": source_identifier_for_chunk_desc,
            "created_at_ts_param": created_at,
            "dynamic_chunk_properties_param": dynamic_chunk_properties,
            "chunk_number_param_for_rel": chunk_number_for_rel if chunk_number_for_rel is not None else 0,
        }
        try:
            results, _, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.ADD_CHUNK_AND_LINK_TO_SOURCE, params, database_=self.database
            )
            if results and results[0]["chunk_uuid"]:
                return results[0]["chunk_uuid"]
            logger.warning(f"NodeManager: ADD_CHUNK_AND_LINK_TO_SOURCE for chunk '{dynamic_chunk_properties.get('name', chunk_uuid)}' did not return a UUID.")
            return None
        except Exception as e:
            if "Failed to invoke procedure `apoc.do.when`" in str(e): 
                logger.error("NodeManager: APOC `apoc.do.when` not found. Ensure APOC plugin is installed.", exc_info=False)
    ...
    async def set_chunk_content_embedding(self, chunk_uuid: str, embedding_vector: List[float]) -> bool:
        try:
            ...
            results, _, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.SET_CHUNK_CONTENT_EMBEDDING, params, database_=self.database
            )
    ...
    async def merge_or_create_entity_node( # Was create_new_entity_with_merge, reverting to this for clarity
        self,
    ...
    async def fetch_entity_details(self, entity_uuid: str) -> Optional[Tuple[str, Optional[str], Optional[str], str]]: # uuid, name, desc, label
        """Fetches name, description, and label for an existing entity by UUID."""
        try:
            results, _, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.GET_ENTITY_DETAILS_FOR_UPDATE, # Ensure this query is in cypher_queries.py
                uuid_param=entity_uuid,
            ...
            if results and results[0]:
                ...
                return (
                    ...
                    record["current_entity_description"],
                    record["entity_label"] 
                )
            logger.warning(f"NodeManager: No details found for entity UUID '{entity_uuid}'.")
            return None
        except Exception as e:
    ...
    async def update_entity_name(self, entity_uuid: str, new_name: str, updated_at_ts: datetime) -> bool:
        try:
            ...
            results, _, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.UPDATE_ENTITY_NAME, params, database_=self.database
            )
    ...
    async def update_entity_description(self, entity_uuid: str, new_description: Optional[str], updated_at_ts: datetime) -> bool:
        try:
            ...
            # Check if description matches, considering nulls
    ...
    async def set_entity_name_embedding(self, entity_uuid: str, embedding_vector: List[float]) -> bool:
        try:
            ...
            results, _, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.SET_ENTITY_NAME_EMBEDDING, params, database_=self.database
            )
    ...
    async def link_chunk_to_entity(
        ...
        try:
            results, _, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.LINK_CHUNK_TO_ENTITY, params, database_=self.database
            )
    ...
    async def create_or_merge_relationship(
        self,
        ...
        params = {
            "source_entity_uuid_param": source_entity_uuid,
            "target_entity_uuid_param": target_entity_uuid,
            "relation_label_param": relation_label,
            "fact_sentence_param": fact_sentence,
            "relationship_uuid_param": relationship_uuid, 
            "source_chunk_uuid_param": source_chunk_uuid, 
            "created_at_ts_param": created_at_ts 
        }
        try:
            results, summary, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.MERGE_RELATIONSHIP,
                params,
                database_=self.database
            )
            ...
            logger.warning(f"NodeManager: MERGE_RELATIONSHIP for '{relation_label}' between '{source_entity_uuid}' and '{target_entity_uuid}' did not return UUID.")
            return None
        except Exception as e:
    ...
    async def set_relationship_fact_embedding(self, relationship_uuid: str, embedding_vector: List[float]) -> bool:
        try:
            ...
            results, _, _ = await self.driver.execute_query( # type: ignore
                cypher_queries.SET_RELATIONSHIP_FACT_EMBEDDING,
                params,
                database_=self.database
            )
